  ...
  struct SeedTruth {
    ...
    bool write(const std::string &fn, float pt_min, float d0_max) const {
      ...
      fprintf(f, "# lo hi  findable found extra_true_quads found_with_dup  quads true fake undecidable"
                 "  true_below_ptmin true_displaced\n");
      for (int b = 0; b < kNb; ++b)
        fprintf(f, "%.2f %.2f  %.0f %.0f %.0f %.0f  %.0f %.0f %.0f %.0f  %.0f %.0f\n", b * kEtaMax / kNb,
                (b + 1) * kEtaMax / kNb, den[b], found[b], dup_extra[b], found_dup[b], q_all[b], q_true[b], q_fake[b],
                q_undec[b], q_true_lowpt[b], q_true_displ[b]);
      fclose(f);
      return true;
    }
